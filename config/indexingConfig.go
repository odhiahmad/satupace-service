...
func SetupIndexes(db *sql.DB) {
	queries := []string{
		// Aktifkan ekstensi pg_trgm
		...
		// Composite indexes (business_id + name)
		...
		// GIN index untuk pencarian LIKE/ILIKE
	...
	for _, q := range queries {
		if _, err := db.Exec(q); err != nil {
			log.Printf("❌ Gagal buat index: %s\nError: %v", q, err)
		} else {
			log.Printf("✅ Index berhasil atau sudah ada: %s", q)
		}
	}
}
